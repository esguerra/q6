...
subroutine pmf_finalize
  ...
  do i = 0,ligand%num_atom -1
    ...
    write(*,'(a,i4,a,a,a,a,a)') 'WARNING: Ligand atom ', &
    ligand%atom(i)%id, ' (', trim(adjustl(ligand%atom(i)%ttype)), ' in residue ', &
    trim(adjustl(ligand%atom(i)%residue)), ') scored zero in all frames.'
      warn = warn +1
    end if
  end do
